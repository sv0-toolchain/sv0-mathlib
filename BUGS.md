...
Filed here first (this repo has no upstream tracker yet); each should become a
real `sv0-toolchain`/`sv0c` issue before R0.1 work goes much further, since
`math::modular` (u64, large moduli) and `math::trig`/`math::complex` (f64,
large iteration counts) depend on all three being fixed. Audited against
sv0-toolchain HEAD `840fe73` / sv0c HEAD `0705d68` (2026-08-23).
...
- Single-file compile/verify (`./scripts/sv0 compile`, `verify`, `emit-c`)
  for `i32`/`bool`/`f64` contracts, staged at a path relative to `sv0c/`.
- Multi-file `--project` builds via the **native** mega-TU compiler
  (`build/sv0-megatu-compiler-native --project <dir>`) for code with **no**
  generic (`<T>`) type declarations — confirmed against this repo's own
  `main.sv0` + `lib/arith.sv0` (`abs_i32`, `abs_f64`, both with `f64`/`i32`
  contracts), compiled, linked (`cc`), and run to the expected exit code.

## Not yet working: VM backend for this project

`./scripts/sv0 vm-project-compile ../../sv0-mathlib` (the SML driver) fails
on this repo's *current, otherwise-green* subset with `E0402: expression
form not supported in type checker slice` — triggered by `abs_f64`'s
`ensures(result >= 0.0)` on an `f64`, consistent with bug #2 above (the VM
path's SML-legacy checker only understands `i32`/`bool` in contract
expressions). So today, this library's C-backend build (native compiler,
`--project`) is ahead of its VM-backend build: **VM parity (COMPAT-001/002)
cannot be exercised for anything beyond pure-`i32`, contract-free code**,
which is nearly this library's whole surface. Re-test once bug #2 is fixed.
